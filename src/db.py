...
def insert_row(report_number, time_reported, date_time_occurred, incident, location, narrative):
    with closing(sqlite3.connect("reports.db")) as connection:
        with closing(connection.cursor()) as cursor:
            cursor.execute("INSERT INTO report (report_number, time_reported, date_time_occurred, incident, location, narrative) values (?, ?, ?, ?, ?, ?)", (report_number, time_reported, date_time_occurred, incident, location, narrative))
        connection.commit()

def insert_report(report):
    insert_row(report['Report Number'], report['Time Reported'], report['Date / Time  Occurred *'], report['Incident'], report['Location'], report['Narrative'])
